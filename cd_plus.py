...
def get_civitai_model_info_by_url(model_url, api_key):
    ...
    match = re.search(
        r"civitai\.com/models/(\d+)(?:\?modelVersionId=(\d+))?", model_url
    )
    if not match:
        ...
        if model_version_id:
            for version in data["modelVersions"]:
                if version["id"] == model_version_id:
                    ...
                    model_info["model_version_download_link"] = version["files"][0][
                        "downloadUrl"
                    ]
            ...
            # 否则提取主版本的下载链接和预览图片链接
            model_info["download_link"] = data["modelVersions"][0]["files"][0][
                "downloadUrl"
            ]
            model_info["preview_image_url"] = data["modelVersions"][0]["images"][0][
                "url"
            ]
...
def upload_to_huggingface(model_name, download_dir, repo_id, repo_type, hf_token):
    ...
    for extension in file_extensions:
        ...
        repo_file_path = (
            f"{model_name}/{local_file_name}"  # 在仓库中将文件上传到与模型名称相同的文件夹下
        )
        ...
        if os.path.exists(local_file_path):
            ...
            print(
                f"文件 '{local_file_path}' 已成功上传到仓库 '{repo_id}' 的 '{repo_file_path}'"
            )
        else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python your_script_name.py <模型地址> <Hugging Face Token> <Civitai API Key> <Hugging Face 仓库 ID> [<下载目录>]"
        )
        sys.exit(1)
    ...
    if model_info:
        # 下载模型, 判断是否带有版本信息
        if model_info.get(
            "model_version_download_link"
        ):  # 优先判断是否有版本下载链接
            download_url = model_info["model_version_download_link"]
        elif model_info.get("download_link"):  # 如果没有版本下载链接，则使用默认下载链接
            download_url = model_info["download_link"]
        else:
            print("没有找到模型下载链接！")
            download_url = None
        ...
        if download_url:
            ...
            if download_url:
                ...
                response = requests.get(
                    download_url, headers={"Range": "bytes=0-0"}
                )  # 只请求第一个字节
                response.raise_for_status()
                default_file_name = get_file_name_from_response(response)
                ...
                if os.path.exists(model_file_path):
                    ...
                    model_file_name = download_file(
                        download_url, download_dir=download_dir
                    )
            ...
            upload_to_huggingface(
                file_name_base, download_dir, repo_id, repo_type, hf_token
            )
